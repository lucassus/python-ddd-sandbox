...
def update_project_endpoint(project_id: int, data: schemas.UpdateProject, bus: MessageBusDep):
...
def archive_project_endpoint(project_id: int, bus: MessageBusDep):
    bus.execute(ArchiveProject(ProjectID(project_id), now=utc_now()))
    return Response(status_code=HTTP_200_OK)


@router.put("/{project_id}/unarchive")
def unarchive_project_endpoint(project_id: int, bus: MessageBusDep):
    bus.execute(UnarchiveProject(ProjectID(project_id)))
    return Response(status_code=HTTP_200_OK)


@router.delete("/{project_id}")
def delete_project_endpoint(project_id: int, bus: MessageBusDep):
